game.BirdEntity = me.Entity.extend({
  ...
  onCollision: function (response) {
    var obj = response.b;
    if (obj.type.startsWith("pipe") || obj.type === "ground") {
...
["1", "2", "3"].forEach((num) => {
  game["Pipe" + num + "Entity"] = me.Entity.extend({
    init: function (x, y) {
      var settings = {};
      settings.image = this.image = me.loader.getImage("pipe-" + num);
      settings.width = 148;
...
game.PipeGenerator = me.Renderable.extend({
  ...
  update: function (dt) {
    if (this.generate++ % this.pipeFrequency == 0) {
      ...
      var pipe1 = new me.pool.pull(randPipe(), this.posX, posY);
      var pipe2 = new me.pool.pull(randPipe(), this.posX, posY2);
      var hitPos = posY - 100;
